...
def calculate_precision_recall(relevant_docs, retrieved_docs):
    ...
    map = sum(avg_precisions) / queries

    return precisions, recalls, map

map_values= []
def plot_precision_recall_for_query(query_id, precisions, recalls):
    plt.figure()
    ranks = range(1, len(precisions) + 1)

    plt.plot(ranks, precisions, marker='o', label='Precision')
    plt.plot(ranks, recalls, marker='x', label='Recall')

    plt.title(f"Precision and Recall at Different Ranks for Query {query_id}")
    plt.xlabel("Rank")
    plt.ylabel("Value")
    plt.legend()
    plt.grid(True)
...
for query_id, relevant_docs_set in relevant_docs.items():
    ...
    precisions, recalls, map= calculate_precision_recall(relevant_docs_set, retrieved_docs_list)
    map_values.append(map)

    plt.figure()
    plt.plot(recalls, precisions, marker='o')
    plt.title(f"Precision vs Recall for Query {query_id + 1}")
    plt.xlabel("Recall")
